...
import { useGameStore, GameConfig, AnswerResult } from './gameStore';
import { 
  useGenerateQuestion, 
  useSubmitAnswer, 
...
export const useGameFlow = () => {
  ...
  const submitAnswer = useCallback(async (selectedAnswer?: string) => {
    ...
    try {
      ...
      const answerResult: AnswerResult = {
        ...
        correctAnswer: result.correctAnswer || state.currentQuestion.correctAnswer,
        selectedAnswer: answer,
        timeTaken,
        explanation: typeof result.explanation === 'string' 
          ? result.explanation 
          : result.explanation?.explanation || undefined,
        streakBroken: !result.isCorrect && state.stats.currentStreak > 0,
        achievement: checkForAchievements(result.isCorrect, result.pointsEarned || 0),
      };
      
      // Update game state
      gameStore.submitAnswer(answerResult);
      
      // Schedule next question if auto-advance is enabled
      const currentState = useGameStore.getState();
      if (currentState.config.autoAdvance) {
        setTimeout(() => {
          generateNextQuestion();
        }, currentState.config.autoAdvanceDelay * 1000);
      }
  ...
  }, [gameStore, submitAnswerMutation, generateNextQuestion, stopQuestionTimer, checkForAchievements]);
...
export const useGameTimer = () => {
  ...
  return {
    ...
    hasTimeExpired: timeRemaining === 0,
  };
};
